...
class Akismet_Admin {
	...
	public static function init_hooks() {
		...
		add_filter( 'plugin_action_links_' . plugin_basename( plugin_dir_path( __FILE__ ) . 'akismet.php' ), array(
			'Akismet_Admin',
		...
		if ( class_exists( 'Jetpack' ) ) {
			add_filter( 'akismet_comment_form_privacy_notice_url_display', array(
				'Akismet_Admin',
				'jetpack_comment_form_privacy_notice_url'
			) );
			add_filter( 'akismet_comment_form_privacy_notice_url_hide', array(
				'Akismet_Admin',
				'jetpack_comment_form_privacy_notice_url'
			) );
		}
		...
		if ( get_option( 'Activated_Akismet' ) ) {
			...
			if ( ! headers_sent() ) {
				wp_redirect( add_query_arg( array(
					...
					'view' => 'start'
				), class_exists( 'Jetpack' ) ? admin_url( 'admin.php' ) : admin_url( 'options-general.php' ) ) );
			}
		}

		load_plugin_textdomain( 'akismet' );
		add_meta_box( 'akismet-status', __( 'Comment History', 'akismet' ), array(
			'Akismet_Admin',
	...
	public static function load_menu() {
		if ( class_exists( 'Jetpack' ) ) {
			$hook = add_submenu_page( 'jetpack', __( 'Akismet Anti-Spam', 'akismet' ), __( 'Akismet Anti-Spam', 'akismet' ), 'manage_options', 'akismet-key-config', array(
				'Akismet_Admin',
				'display_page'
			) );
		} else {
			$hook = add_options_page( __( 'Akismet Anti-Spam', 'akismet' ), __( 'Akismet Anti-Spam', 'akismet' ), 'manage_options', 'akismet-key-config', array(
				'Akismet_Admin',
				'display_page'
			) );
		}
	...
	public static function check_for_spam_button( $comment_status ) {
		...
		echo '<a
			...
			), add_query_arg( array( 'akismet_recheck_error' => 1 ) ) ) ) . '"
	...
	public static function get_akismet_user( $api_key ) {
		...
		$subscription_verification = Akismet::http_post( Akismet::build_query( array(
			'key'  => $api_key,
	...
	public static function display_configuration_page() {
		...
		if ( empty( self::$notices ) ) {
			...
			if ( ! empty( $akismet_user->limit_reached ) && in_array( $akismet_user->limit_reached, array(
					'yellow',
					'red'
				) ) ) {
		...
		if ( ! isset( self::$notices['status'] ) && in_array( $akismet_user->status, array(
				'cancelled',
				'suspended',
				'missing',
				'no-sub'
