...
<metaslider-shortcode inline-template>
    <div class="shadow-sm bg-white mb-6 flex flex-col">
        ...
        <div class="m-3 mt-2">
            ...
            <pre id="shortcode" ref="shortcode" dir="ltr" class="text-gray text-sm">&lt;?php echo do_shortcode('<br>&emsp;&emsp;<div
                        @click.prevent="copyShortcode($event)"
                        class="text-orange cursor-pointer whitespace-normal inline">[metaslider <template
                            v-if="useTitle">title="{{ current.title }}"</template><template
                            v-else>id="{{ current.id }}"</template>]</div><br>'); ?&gt;</pre>
